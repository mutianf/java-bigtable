...
public class DelayedClientCall<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    protected DelayedClientCall(
        ...
        System.out.println("HELLO FROM DELAYED CLEITN CALL");
    ...
    private void cancel(final Status status, boolean onlyCancelPendingCall) {
        System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling");
        boolean delegateToRealCall = true;
        Listener<RespT> listenerToClose = null;
        synchronized (this) {
            // If realCall != null, then either setCall() or cancel() has been called
            if (realCall == null) {
                System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling, real call is null");
                ...
                System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling, only cancel pending call");
                return;
            } else {
                System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: :O");
            }
        }
        if (delegateToRealCall) {
            System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling, delegating to real call");
            delayOrExecute(new Runnable() {
                @Override
                public void run() {
                    System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling, run!!! ");
                    realCall.cancel(status.getDescription(), status.getCause());
                }
            });
        } else {
            System.out.println(Thread.currentThread().getId() + " - DelayedCleintCall: cancelling, not delelagting ");
