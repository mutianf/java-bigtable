...
final class ClientCallImpl<ReqT, RespT> extends ClientCall<ReqT, RespT> {
    ...
    ClientCallImpl(
        ...
        System.out.println("CREATING A NEW CLIENT CALL IMPL:" + Arrays.toString(Thread.currentThread().getStackTrace()));
        this.method = method;
    ...
    private void startInternal(Listener<RespT> observer, Metadata headers) {
        System.out.println(Thread.currentThread().getId() + " - ClientCallImpl: startInternal is called, context isCancelled=" + context.isCancelled());
        System.out.println("cancelled is " + cancelCalled + " stream: " + stream);
        checkState(stream == null, "Already started");
        if (cancelCalled) {
            System.out.println("I AM CANCELLED");
            new Exception("aa").printStackTrace();
            throw new RuntimeException(" I  AM CANCELLED!");
        }
        checkState(!cancelCalled, "call was cancelled");
        checkNotNull(observer, "observer");
        checkNotNull(headers, "headers");
        System.out.println(Thread.currentThread().getId() + " - ClientCallImpl: check state passed");

        if (context.isCancelled()) {
            System.out.println(Thread.currentThread().getId() + " - ClientCallImpl: context is cancelled");
        ...
        if (compressorName != null) {
            ...
            System.out.println("here: " + compressor);
            if (compressor == null) {
                stream = NoopClientStream.INSTANCE;
                final Listener<RespT> finalObserver = observer;
    ...
    private void cancelInternal(@Nullable String message, @Nullable Throwable cause) {
        if (message == null && cause == null) {
            cause = new CancellationException(Thread.currentThread().getId() + " -  ClientCallImpl: Cancelled without a message or cause");
        ...
        try {
            ...
            System.out.println(Thread.currentThread().getId() + " - ClientCallImpl: cancelInternal is called, stream is " + stream);
            if (stream != null) {
    ...
    private void sendMessageInternal(ReqT message) {
        System.out.println(Thread.currentThread().getId() + " - ClientCallImpl: sendMessage is called, stream is " + stream + Arrays.toString(Thread.currentThread().getStackTrace()));
        checkState(stream != null, "Not started");
        checkState(!cancelCalled, "call was cancelled");
